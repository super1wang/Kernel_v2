...
def identity(profile,mode):
    paths=[ROOT/'dependencies.lock',ROOT/'CMakeLists.txt',ROOT/'CMakePresets.json',ROOT/'tools/evidence/process.py']
    for base in ('packages','cmake','sdk','tools/footprint','tests/contract/native'):
    ...
    value={'profile':profile,'mode':mode,'stage':'NativeSubset','architecture':'x64',
           'os':platform.platform(),'cpu':platform.processor(),'logical_cpus':os.cpu_count(),'python':platform.python_version(),
...
def measure(args):
    ...
    try:
        ...
        for k,p in binaries.items():
            ...
            traces=list((build/('footprint_'+k+'.dir')/configuration).glob('*.tlog/CL.command.1.tlog'))
            if len(traces)!=1:raise ValueError('missing actual pair compiler trace')
            trace=traces[0].read_text(encoding='utf-16').lower()
            if (set(re.findall(r'/(mdd|md|mtd|mt)\b',trace))!={'mdd' if configuration=='Debug' else 'md'}
                or ('/fsanitize=address' in trace)!=asan or re.search(r'/gl\b',trace)):
                raise ValueError('pair CRT/ASan/LTO mismatch')
            shutil.copyfile(traces[0],out/(k+'-CL.command.tlog'))
        ...
        for b in range(blocks):
            for index,k in enumerate(('baseline','native','native','baseline')):
                ...
                for begin,end in zip(obs['phases'],obs['phases'][1:]):
                    ...
                    if not points:raise ValueError('missing phase measurement window')
                    windows[begin['phase']]={name:statistics([s[name] for s in points]) for name in ('private_bytes','working_set_bytes')}
                rows.append({'run_id':str(r['pid'])+'-'+label+'-'+out.name,'command':label,'block':b,
                    ...
                    'create_to_ready_ms':obs['ready']['create_to_ready_ticks']*1000/obs['qpc_frequency'],
                    'ready_peak_commit':obs['ready']['peak_commit_bytes'],
                    'ready_peak_working_set':obs['ready']['peak_working_set_bytes'],
                    'sample_count':len(samples),'missed_intervals':sum(s['missed_intervals'] for s in samples),
                    'windows':windows,
                    'thread_peak':max(len(s['thread_ids']) for s in samples if s.get('thread_ids') is not None),
                    'thread_boundaries':{s['reason']:len(s['thread_ids']) for s in samples if s.get('thread_ids') is not None},
        ...
        for kind in ('baseline','native','held_thread'):
            binary=diagnostic/configuration/('footprint_'+kind+'.exe')
            shutil.copyfile(binary,out/('thread-'+kind+'.exe'))
            r=run('thread-'+kind,[str(binary)],replace(observation,mode='latency'))
        ...
        thread_delta=paired_differences([{**r,'value':r['thread_peak']} for r in rows],blocks)
        if any(v!=0 for v in thread_delta):raise ValueError('each paired native thread increment must be zero')
        for phase in ('assigned_suspended',*PHASES):
            if any(v!=0 for v in paired_differences([{**r,'value':r['thread_boundaries'][phase]} for r in rows],blocks)):
                raise ValueError('paired thread boundary differs: '+phase)
        metrics={}
        keys=('internal_ms','create_to_ready_ms') if mode=='latency' else ('private_peak','working_set_peak','ready_private','ready_peak_commit','ready_peak_working_set')
        stats={}
        for key in keys:
