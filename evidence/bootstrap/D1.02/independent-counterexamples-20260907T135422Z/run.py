...
source=ROOT/'build/d1.02-review-probes'
evidence=ROOT/'evidence/bootstrap/D1.02'/('independent-counterexamples-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ'))
evidence.mkdir()
...
for mode in ['control','effect','transition','caller-injection','proof-injection']: run(mode,[exe,mode])
run('private-issuance-red',[cmake,'--build',str(out),'--config','Debug','--target','private_issuance','--','/m:1','/nr:false'])
