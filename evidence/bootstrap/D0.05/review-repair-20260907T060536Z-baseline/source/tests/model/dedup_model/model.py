...
class Request:
    ...
    def fingerprint(self):
        ...
        return (self.operation, self.contract, self.args, self.target, self.preconditions, self.plan_digest)
    ...
    def claim(self, session, namespace, epoch, nonce, request, crash_at=None):
        ...
        with self.writer():
            ...
            record = {'key': key, 'fingerprint': fingerprint, 'execution_ref': execution_ref,
                      'status': 'Claimed', 'outcome': None, 'pins': set(), 'references': set()}
            operations = [
    ...
    def restart(self):
        ...
        if self.volatile:
            ...
            for record in self.image['records'].values():
                if record['status'] == 'Claimed': record['status'] = 'Suspended'
                elif record['status'] == 'EffectPermitted': record['status'] = 'Unknown'
    ...
    def authorize_effect_send(self, session, key):
        ...
        with self.writer():
            ...
            require(isinstance(key, IntentKey) and key.scope == self._scope(session, key.scope.namespace),
                    '旧世代/其他主体历史不能换范围重播')
            record = self.image['records'].get(key)
            require(record is not None and record['status'] == 'Claimed' and record['outcome'] is None,
