...
class CommitDomain:
    ...
    def outcome(self, attempt):
        ...
        if attempt.phase in ('Published', 'Finalized'):
            return dict(common, kind='StateCommitted', commit_id=attempt.commit_id, domain='domain-1',
                        revision=str(attempt.prepared.revision), published_version=str(attempt.prepared.revision),
                        result={'value': attempt.prepared.value})
...
class Effect:
    ...
    def send(self, application, status):
        ...
        self.report = self.device.apply(self.effect_id, application, status)

    def _resolved(self, report, result_valid=True):
        self.facts.append({'kind': 'EffectFact', 'fact_id': 'effect:' + self.effect_id,
                           'effect_id': self.effect_id, 'application': report['application']})
        result = {'kind': 'EffectResolved', 'evidence': 'Durable', 'effect_id': self.effect_id,
                  **report, 'known_facts': deepcopy(self.facts)}
        if result_valid: result['result'] = {'receipt': self.effect_id}
        else: result['result_error'] = 'ResultEncodingFailed'
        return result

    def record(self, result_valid=True):
        require(self.report is not None and self.disk['outcome'] is None and not self.recovered, '没有可保存的实际效果报告')
        self.disk['outcome'] = self._resolved(self.report, result_valid)

    def restart(self):
        self.permitted, self.report, self.recovered = False, None, True
        if self.disk['claim'] is not None and self.disk['outcome'] is None:
            self.facts = [{'kind': 'UnknownFact', 'fact_id': 'unknown:' + self.effect_id,
                           'boundary': 'ExternalEffect', 'reference_id': self.effect_id, 'reconcile': 'InspectDeviceLedger'}]
    ...
    def reconcile(self):
        ...
        report = self.device.receipts.get(self.effect_id)
        require(report is not None, '未找到 outcome 不能证明未发送；需要外部证据')
        result = self._resolved(report)
        self.facts.append({'kind': 'ResolutionRecord', 'fact_id': 'resolve:' + self.effect_id,
                           'unknown_id': 'unknown:' + self.effect_id, 'determination': report['application'],
    ...
    def outcome(self):
        ...
        if self.recovered and self.disk['claim'] is not None:
            return {'kind': 'Indeterminate', 'evidence': 'Durable', 'known_facts': deepcopy(self.facts),
                    'unknown_ids': ['unknown:' + self.effect_id]}
        return None
