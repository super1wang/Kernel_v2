...
完整的 54 项固定预期、精确命令和严格运行入口见 [manifest](../../tests/manifests/d0.05.expected.json) 与 [commit.md](commit.md)。bootstrap 运行事实位于 [D0.05 evidence](../../evidence/bootstrap/D0.05/)，正式采集/独立审查/用户批准仍分别由相应流程完成。该模型不新增 Runtime、产品模块、旧数据迁移、分布式事务或通用形式化框架。
