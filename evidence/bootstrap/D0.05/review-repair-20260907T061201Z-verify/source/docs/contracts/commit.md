...
固定清单 [d0.05.expected.json](../../tests/manifests/d0.05.expected.json) 合计 62 个实际用例（提交/许可/效果 33；去重/恢复 29），逐条使用 `test_Txx_component_behavior` 命名。清单独立维护，测试只比较预期与 AST 发现结果，不用发现结果重写预期。
