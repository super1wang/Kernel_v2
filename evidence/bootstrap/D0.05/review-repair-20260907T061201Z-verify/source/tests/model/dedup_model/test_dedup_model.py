...
class DedupContracts(unittest.TestCase):
    ...
    def test_T15_intent_request_symbol_values_are_owned(self):
        s,session,req=self.ready();args={'nested':[1,2]};req=replace(req,args=(args,));first=s.claim(session,'default',1,'n',req);stored=deepcopy(s.image)
        args['nested'][0]=99
        self.assertEqual(s.image,stored)
        with self.assertRaises(ContractError):s.claim(session,'default',1,'n',req)
        self.assertEqual(s.image['inputs'][first['execution_ref']].args,({'nested':[1,2]},))

        # 重算普通备份hash不能掩盖输入与接受指纹不一致。
        from dedup_model.model import digest
        backup=s.backup();backup['image']['inputs'][first['execution_ref']]=replace(req,args=({'nested':[999]},))
        backup['image_sha256']=digest(backup['image'])
        before=(s.restore_generation,deepcopy(s.image))
        with self.assertRaises(ContractError):s.restore_backup(backup)
        self.assertEqual((s.restore_generation,s.image),before)

    def test_T15_intent_fingerprint_preserves_scalar_types(self):
        for before,after in [(1,True),(1,1.0),(0.0,-0.0)]:
            s,session,req=self.ready();s.claim(session,'default',1,'n',replace(req,args=(before,)))
            with self.assertRaises(ContractError):s.claim(session,'default',1,'n',replace(req,args=(after,)))
        for invalid in [float('nan'),float('inf'),object()]:
            s,session,req=self.ready()
            with self.assertRaises(ContractError):s.claim(session,'default',1,'n',replace(req,args=(invalid,)))
            self.assertEqual(s.image['records'],{})

    def test_T18_restore_unknown_effect_cannot_reenter_claimed(self):
        for restart in (False,True):
            s,session,req=self.ready();record=s.claim(session,'default',1,'n',req);key=record['key'];s.authorize_effect_send(session,key)
            if restart:s.restart();session=s.connect('alice')
            original=deepcopy(s.image)
            for status in ('Claimed','Suspended'):
                with self.assertRaises(ContractError):s.set_status(key,status)
                self.assertEqual(s.image,original)
            # 错误投影值也不能覆盖独立发送准入事实。
            s.image['records'][key]['status']='Claimed'
            with self.assertRaises(ContractError):s.authorize_effect_send(session,key)
            self.assertEqual(s.effect_dispatches,[key])
