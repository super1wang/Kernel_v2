...
commands=[]
def run(n,a):
 r=execute(a,ROOT,out/(n+'-stdout.log'),out/(n+'-stderr.log'),600);commands.append(r);(out/'commands.json').write_text(json.dumps(commands,indent=2));print(n,r['status'],r['exit_code'],str(out),flush=True);return r['exit_code']==0
