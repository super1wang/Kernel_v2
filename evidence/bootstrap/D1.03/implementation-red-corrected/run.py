...
# preserved preimplementation source snapshot
...
incs=[out,ROOT/'tests/compile/contracts',ROOT/'packages/contracts/include',ROOT/'packages/foundation/include',ROOT/'build/d0.06-a/cache/sources/expected-fe3b18aecb84/include']
...
for n,s in [('positive',out/'positive.cpp'),('negative',out/'registration_tests.cpp')]:
 lines += [f'add_library({n} OBJECT EXCLUDE_FROM_ALL "{s.as_posix()}")',f'target_compile_features({n} PRIVATE cxx_std_20)',f'target_compile_options({n} PRIVATE /utf-8 /EHsc /permissive- /Zc:__cplusplus)',f'target_include_directories({n} PRIVATE '+ ' '.join('"'+p.as_posix()+'"' for p in incs)+')']
...
b=ROOT/'build/d1.03-implementation-red-corrected'
