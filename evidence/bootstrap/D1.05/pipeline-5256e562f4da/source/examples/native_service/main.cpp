...
PrincipalRef principal() { return {identity<foundation::PrincipalId>()}; }
...
int main() {
  ...
  try {
    ...
    require(std::get<Rejected>(invalid).error.code() == error(ContractsErrc::InvalidContract).code());
    require(entered == before);
    auto unavailable = editing->invoke(Value{5}, options);
    require(std::holds_alternative<Rejected>(unavailable));
    require(std::get<Rejected>(unavailable).error.code() == invocation_error(InvocationErrc::ProviderUnavailable).code());
    require(entered == before);
