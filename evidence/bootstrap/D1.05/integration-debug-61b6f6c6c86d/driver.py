"""完整D1.05集成开发轮；仅在实现就绪后运行，不替代正式矩阵。"""
...
out = ROOT / 'evidence/bootstrap/D1.05' / ('integration-' + profile + '-' + uuid.uuid4().hex[:12])
out.mkdir(parents=True, exist_ok=False)
spec_path = f'tests/runs/d1.05-win-msvc-{profile}.json'
spec = read_json(spec_path)
rows = inputs(ROOT, spec, spec_path)
frozen_expected = sorted(c['id'] for c in read_json(spec['expected_manifest'])['cases'] if '.native.' in c['id'])
assert len(frozen_expected) == 32
...
build = 'build/d1.05-integration-' + uuid.uuid4().hex[:8]
commands = []
...
def archive_materials():
    base = ROOT / build / 'tests/contract/native'
...
try:
    ...
    binary = ROOT / build / f'tests/contract/native/{spec["configuration"]}/ock_native_tests.exe'
    registry = ROOT / build / f'tests/contract/native/native-tests-{spec["configuration"]}.cmake'
    ...
    assert len(expected) == 32 and sorted(actual_list) == expected
    junit = out / 'native-junit.xml'
    run('ctest-native', ['ctest', '--test-dir', build, '-C', spec['configuration'], '-R', '^T(02|03|06|23)[.]native[.]', '--output-on-failure', '--no-tests=error', '--output-junit', str(junit)])
