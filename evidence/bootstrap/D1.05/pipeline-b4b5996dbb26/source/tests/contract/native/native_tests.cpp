...
void invalid_input() { Env e;auto b=e.bind();CHECK(b);entered=0;CHECK(std::holds_alternative<Rejected>(b->invoke(-1,e.options())));CHECK(entered==0);CHECK(result(b->invoke(0,e.options()))==2); }
...
int main(int argc,char**argv) {
  const std::map<std::string,void(*)()> cases{{"T02.native.registered_compute",registered_compute},{"T02.native.registered_read",registered_read},{"T02.native.invalid_input",invalid_input},{"T06.native.authority_origin",authority_origin},
    ...
    {"T03.native.reentrant_and_concurrent",binding_capacity}};
