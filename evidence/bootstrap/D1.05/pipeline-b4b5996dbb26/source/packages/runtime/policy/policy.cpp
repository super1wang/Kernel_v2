...
Result<TimePoint> SessionAuthority::authorize_inline(
    const VerifiedCaller &caller, const DefinitionSnapshot &definition,
    std::span<const ObjectId> targets, TimePoint deadline) const {
  auto session = state_->value->session;
  auto store = session->hold.store;
  auto verified = detail::Access::record(caller);
  if (verified->session != session || targets.empty())
    return failure<TimePoint>(PolicyErrc::InvalidAuthority);
  std::lock_guard lock(store->mutex);
  auto live = detail::current(*session);
  if (!live)
    return make_unexpected(live.error());
  const auto effective_deadline = (std::min)(deadline, session->deadline);
  if (store->clock->now() >= effective_deadline)
    return failure<TimePoint>(PolicyErrc::Expired);
  auto *grant = dynamic_cast<const detail::Grant *>(verified->grant.get());
  if (!grant || grant->session != session || grant->issuer != session->caller_port ||
      grant->generation != session->generation)
    return failure<TimePoint>(PolicyErrc::InvalidAuthority);
  const OperationSelector selector{definition.description().key,
                                   definition.description().contract_digest};
  auto *installed = detail::operation_policy(*store, selector);
  if (!installed)
    return failure<TimePoint>(PolicyErrc::PolicyNotInstalled);
  const auto &required = definition.description().required_permissions;
  if (installed->required_permissions != required)
    return failure<TimePoint>(PolicyErrc::ContractMismatch);
  for (auto target : targets) {
    auto stamped = detail::stamp(*store, target);
    if (!stamped || !detail::stamp_current(*store, *stamped))
      return failure<TimePoint>(PolicyErrc::TargetUnavailable);
    if (!detail::allowed(*session, AccessUse::Invoke, selector, target,
                         session->principal))
      return failure<TimePoint>(PolicyErrc::Denied);
  }
