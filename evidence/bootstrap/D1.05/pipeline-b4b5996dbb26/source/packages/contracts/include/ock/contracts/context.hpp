...
public:
  ...
  WorkContext(std::stop_token stop,
      ...
      : stop_(stop), deadline_(deadline), budget_(budget), trace_(trace),
        borrowed_views_(resources.values) {
    for (const auto *resource : borrowed_views_)
  ...
  std::span<const ResourceLease *const> granted_resources() const noexcept {
    return resources_ ? std::span<const ResourceLease *const>(owned_views_)
                      : borrowed_views_;
  }
...
private:
  ...
  Name trace_;
  std::optional<std::vector<std::shared_ptr<ResourceLease>>> resources_;
  std::vector<const ResourceLease *> owned_views_;
  std::span<const ResourceLease *const> borrowed_views_;
};
