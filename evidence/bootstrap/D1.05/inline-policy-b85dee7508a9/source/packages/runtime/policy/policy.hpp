...
public:
  ...
  Result<std::shared_ptr<ActionAuthorization>> prepare(const VerifiedCaller &,
                                                       const ActionRequest &);
  // D1.05：只读 Native 调用的每次短仲裁；不发行可复用 Permit。
  // 返回服务器收窄后的有效截止时间；调用者必须将它传入 WorkContext。
  Result<TimePoint> authorize_inline(const VerifiedCaller &,
                                     const DefinitionSnapshot &,
                                     std::span<const ObjectId>, TimePoint) const;
