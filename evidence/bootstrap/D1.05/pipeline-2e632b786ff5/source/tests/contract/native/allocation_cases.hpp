...
struct AllocationReport {
  ...
  void emit(const char* test,bool checked) const {
    ...
    std::cout<<"\"cpp_new\":true,\"debug_crt\":"<<(allocation::crt_available()?"true":"false")
      ...
      <<"\"retained_channel\":\"executable replacement C++ new/delete CRT usable bytes; allocator metadata and DLL/private heaps excluded; no concurrent background activity in these samples\"},\"samples\":[";
...
inline void allocation_probe() {
  ...
  for(int kind=0;kind<11;++kind){allocation::start();void* p=nullptr;
    ...
    if(allocation::crt_available())CHECK(c.crt>0);report.add(names[kind],c);
  }
...
inline void allocation_steady() {
  ...
  for(int n=0;n<40;++n){auto options=e.options();bool success=false;
    ...
    auto c=allocation::stop();report.add("complete_steady_invoke",c,success);all=all&&success&&zero(c)
      &&c.frees==0&&c.live_bytes_before==c.live_bytes_after&&c.peak_live_bytes==c.live_bytes_before;
  }
...
inline void allocation_other_costs() {
  ...
  report.add("final_bound_engine_catalog_observation_reclaimed",count,reclaimed);CHECK(reclaimed);
