...
inline void transmission_start_arbitration() {
  // 只在原返回缓冲仍存活时改变元素；提前复制并释放同样是合法的独立拥有。
  struct AliasingEncoder final : ProjectionEncoderPort {
    ...
    Result<std::vector<std::byte>> encode(const ProjectionSnapshot& p, std::size_t cap) override {
      ...
      authority_cases::observed_buffer = reinterpret_cast<std::uintptr_t>(returned_element);
      authority_cases::observed_buffer_freed = false;
      return std::move(bytes);
    }
  };
  for (bool use_watch : {false, true}) {
    authority_cases::StopObserving guard;
    {
      std::vector<std::byte> positive{std::byte{0x51}, std::byte{0}};
      authority_cases::observed_buffer = reinterpret_cast<std::uintptr_t>(positive.data());
      authority_cases::observed_buffer_freed = false;
    }
    require(authority_cases::observed_buffer_freed, "encoder buffer delete observer positive control");
    authority_cases::observed_buffer = 0;
    Env env;
    ...
    bool changed = false, released_before_reserve = false;
    sink->before_reserve_return = [&] {
      CHECK(encoder->returned_element != nullptr);
      released_before_reserve = authority_cases::observed_buffer_freed;
      if (!released_before_reserve) {
    ...
    if (use_watch) {
      ...
      CHECK((changed || released_before_reserve) && sink->size == 0);
