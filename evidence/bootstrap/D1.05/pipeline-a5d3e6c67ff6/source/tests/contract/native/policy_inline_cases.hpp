...
inline void metadata_budget() {
  ...
  budget.active_inline_calls = budget.declarations;
  Env declarations(budget);
  auto declaration_targets = targets(declarations);
  error_is(declarations.session->prepare_inline(*declarations.caller, d,
      declaration_targets, budget.declarations), PolicyErrc::BudgetExceeded);
  CHECK(declarations.session->prepare_inline(*declarations.caller, d,
      declaration_targets, 1));
}
