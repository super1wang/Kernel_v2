...
Result<std::shared_ptr<NativeEngine>> NativeEngine::create(
  ...
  if(budget.observation_capacity>max/sizeof(std::optional<InvocationRecord>) ||
     budget.concurrent_calls_per_binding>max/sizeof(detail::CallSlot) ||
     budget.targets_per_binding>max/sizeof(foundation::ObjectId) ||
     budget.concurrent_calls_per_binding>max/budget.targets_per_binding ||
     budget.bindings>max/budget.concurrent_calls_per_binding ||
     budget.targets_per_binding>max/(budget.bindings*budget.concurrent_calls_per_binding))
