...
public:
  ...
  InvokeReply<R> invoke(const A& args, const InvokeOptions& options) const {
    ...
    const auto rejected = [&](Error error) -> InvokeReply<R> {
      state->observe(InvocationRecordKind::Rejected, error.code());
    ...
    const auto failed = [&](Error error) -> InvokeReply<R> {
      ...
      state->observe(InvocationRecordKind::FailedBeforeApply, error.code());
    ...
    try {
      ...
      auto count = projection_(args, actual);
      ...
      state->observe(InvocationRecordKind::ReadCompleted, {});
      return reply;
