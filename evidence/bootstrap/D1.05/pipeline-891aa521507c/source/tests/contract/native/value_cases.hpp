...
inline std::function<void()> output_hook;
inline std::function<void()> input_hook;
}
namespace ock::contracts {
template<> struct TypeContract<native_test::Args> {
  static TypeIdentity identity() { return {name("native.args"),ver(native_test::different_identity?"2.0.0":"123456789012345678901234567890.0.0"),{}}; }
  static Result<void> validate(const native_test::Args& a) {
    ++native_test::input_checks;
    if(native_test::input_hook)native_test::input_hook();
...
template<> struct TypeContract<native_test::Value> {
  ...
  static Result<void> validate(const native_test::Value& r) {
    ...
    if(native_test::output_hook)native_test::output_hook();
...
inline void exact_binding() {
  ...
  auto digest=ContractDigest{};digest.bytes[0]=std::byte{1};
...
struct TargetReader {
  int get(foundation::ObjectId target) const {return target==policy_test::target()?11:22;}
};
inline Result<Value> target_read(const Args& a,WorkContext&,ReadServices<TargetReader>& service) {
  ++entered;return Value{service.reader().get(a.target),0};
}
inline void target_read_association() {
  auto service=std::make_shared<TargetReader>();std::weak_ptr<TargetReader> weak=service;
  Env e(true,compute,{},[&](registry::ModuleInput& m){m.services.clear();m.manifest.required_services.clear();
    m.services.push_back(*registry::ServiceBinding::make(name("reader"),service));
    m.manifest.required_services.push_back({{name("native"),name("reader")},CppTypeToken::of<TargetReader>()});
    m.register_operations=[](registry::Registrar& r){auto o=native_options();o.read_service=registry::ServiceRef{name("native"),name("reader")};
      CHECK(r.read(target_read,native_definition(AtomicMode::Incompatible),o));};});
  auto a=bind_value(e),b=bind_value(e,policy_test::target(2));CHECK(a&&b);service.reset();CHECK(!weak.expired());
  for(int n:{1,2}){auto reply=(n==1?a:b)->invoke({0,policy_test::target(n)},e.options());
    CHECK(std::get<ReadCompleted<Value>>(std::get<Completed<Value>>(reply).outcome.value()).result->value==n*11);}
  entered=0;CHECK(std::holds_alternative<Rejected>(a->invoke({0,policy_test::target(2)},e.options())));CHECK(entered==0);
}
...
inline void thread_modes() {
  ...
  for(int mode:{0,1,2}) {
    Env restricted(true,compute,{},[mode](registry::ModuleInput& m){
      m.executors[0].async_dispatch=true;m.executors[0].external_wait=true;
      m.register_operations=[mode](registry::Registrar& r){auto d=native_definition(AtomicMode::Incompatible);
        d.execution.inline_safe=mode!=0;d.execution.requires_async_dispatch=mode==1;d.execution.requires_external_wait=mode==2;
        auto o=native_options();o.read_service=registry::ServiceRef{name("native"),name("reader")};
        CHECK(r.read(read,d,o));};});
...
inline void reentrant_and_concurrent() {
  ...
  Env typed(false,compute,{},value_registration);auto value=bind_value(typed);CHECK(value);
  output_hook=[&]{CHECK(std::holds_alternative<Rejected>(value->invoke({1},typed.options())));};
  CHECK(std::holds_alternative<Completed<Value>>(value->invoke({1},typed.options())));output_hook={};
  CHECK(std::holds_alternative<Completed<Value>>(value->invoke({2},typed.options())));
}
}
