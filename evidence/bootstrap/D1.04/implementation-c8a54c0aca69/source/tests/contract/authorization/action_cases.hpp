...
inline void permit_origin_binding() {
  ...
  Records records;
  auto swapped = EffectContext::check(env.session->callers(), env.session->targets(),
      env.caller->view(), *resolved, second.permit, first.expected, work, records);
  CHECK(swapped);
  CHECK(!first.action->consume(*second.permit, first.expected));
  CHECK(records.calls == 0);
  auto original = EffectContext::check(env.session->callers(), env.session->targets(),
      env.caller->view(), *resolved, first.permit, first.expected, work, records);
  CHECK(original);
  Counter receiver;
  CHECK(receiver.attempt(first));
  CHECK(receiver.attempt(second));
  CHECK(receiver.attempts == 2);
}
