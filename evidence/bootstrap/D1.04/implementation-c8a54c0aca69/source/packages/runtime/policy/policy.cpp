...
Result<std::unique_ptr<SendCoordinator>>SendCoordinator::create(std::shared_ptr<SessionAuthority>authority,std::shared_ptr<TransmissionStartPort>sink,std::shared_ptr<ProjectionEncoderPort>encoder){
 try{if(!owned(authority)||!owned(sink)||!owned(encoder))return failure<std::unique_ptr<SendCoordinator>>(PolicyErrc::InvalidOwner);auto s=Access::record(*authority)->session;auto c=std::make_shared<Coordinator>();c->session=s;c->sink=std::move(sink);c->encoder=std::move(encoder);c->queue.reserve(s->hold.store->budget.queued_frames);auto result=Access::make_unique<SendCoordinator>(c);{std::lock_guard lock(s->hold.store->mutex);auto check=current(*s);if(!check)return make_unexpected(check.error());}return result;
...
Result<void>SendCoordinator::enqueue_response(std::shared_ptr<const ResponseAuthorization>response){
 try{
  ...
  {std::lock_guard lock(s->mutex);check=response_current(*r);if(!check)return check;if(r->status!=ResponseStatus::Fresh)return deny(PolicyErrc::InvalidInput);if(s->queued>=s->budget.queued_frames||f->transmission->bytes().size()>s->budget.queued_bytes-s->bytes)return deny(PolicyErrc::BudgetExceeded);c->queue.push_back(f);++s->queued;s->bytes+=f->transmission->bytes().size();f->charged=true;r->status=ResponseStatus::Queued;}return {};
 }catch(const std::bad_alloc&){return deny(PolicyErrc::BudgetExceeded);}
}
Result<StartResult>SendCoordinator::start_next(){
 ...
 auto source=source_entries(c->session,f->entries);Result<StartResult>result=StartResult::NotStarted;
 {std::lock_guard lock(s->mutex);auto r=Access::record(*f->response);auto check=response_current(*r);if(!source)check=make_unexpected(source.error());
  if(!check){r->status=ResponseStatus::Dropped;result=make_unexpected(check.error());}
  else{auto started=c->sink->start_now(*f->transmission,*f->reservation);result=started;if(started==StartResult::NotStarted)return result;r->status=started==StartResult::Started?ResponseStatus::Started:ResponseStatus::Unknown;if(started==StartResult::Unknown)c->session->closed=true;}
