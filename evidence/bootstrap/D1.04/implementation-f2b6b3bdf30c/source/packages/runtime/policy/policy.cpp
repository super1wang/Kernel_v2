...
struct Response {Hold hold;ResponseId id;std::shared_ptr<Session>session;AccessUse use;TimePoint deadline;std::shared_ptr<const ProjectionSnapshot>projection;std::vector<Entry>entries=std::vector<Entry>(0);ObservationSourceIdentity source;std::uint64_t permission,delegation;ResponseStatus status=ResponseStatus::Fresh;};
struct Watch {Hold hold;WatchKey key;std::uint64_t generation=1;std::shared_ptr<Session>session;ObservationFilter filter{std::vector<ExecutionRef>(0),{},std::vector<ObservationTopic>(0)};TimePoint deadline;ObservationSourceIdentity source;std::vector<Entry>entries=std::vector<Entry>(0);bool closed=false;~Watch(){if(hold.counted){std::lock_guard lock(hold.store->mutex);--session->watches;for(auto&i:hold.store->watch_counts)if(i.first==session->principal){--i.second;break;}}}};
struct Observation {std::shared_ptr<Session>session;};
struct Frame {std::shared_ptr<Frame>retired_next;Hold hold;std::shared_ptr<const ResponseAuthorization>response;std::shared_ptr<Watch>watch;std::shared_ptr<const ProjectionSnapshot>projection;std::vector<Entry>entries=std::vector<Entry>(0);std::unique_ptr<PreparedTransmission>transmission;std::shared_ptr<TransmissionStartPort>reservation_owner;std::unique_ptr<TransmissionReservation>reservation;TimePoint deadline;std::uint64_t permission,delegation;bool charged=false;};
