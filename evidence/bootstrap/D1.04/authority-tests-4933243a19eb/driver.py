...
out=ROOT/'evidence/bootstrap/D1.04'/('authority-tests-'+uuid.uuid4().hex[:12]);out.mkdir()
source=out/'source';rows=[]
for name in ['packages/runtime/policy/policy.hpp','packages/runtime/policy/policy.cpp','tests/contract/authorization/fixtures.hpp','tests/contract/authorization/authority_cases.hpp','tests/compile/contracts/test_support.hpp']:
    p=source/name;p.parent.mkdir(parents=True,exist_ok=True);p.write_bytes((ROOT/name).read_bytes())
    if '--move-fault' in sys.argv and name=='packages/runtime/policy/policy.cpp':
        original=p.read_bytes();(out/'original-policy.cpp').write_bytes(original)
        old=b'std::make_shared<const DelegationInput>(v.ceiling)'
        assert original.count(old)==1
        p.write_bytes(original.replace(old,b'std::make_shared<const DelegationInput>(std::move(v.ceiling))'))
...
names=['authentication_source','scope_four_way','inputs_ownership']
main='''#include "tests/contract/authorization/authority_cases.hpp"
#include <cstdlib>
...
'''
for n in names:main+='if(name=="'+n+'"){policy_test::authority_cases::'+n+'();return 0;}\n'
...
build=ROOT/'build'/('d1.04-authority-'+uuid.uuid4().hex[:8]);commands=[]
def run(name,argv):
...
save_json(out/'result.json',dict(status='Passed' if ok else 'Failed',kind='three-authority-bootstrap-tests',source=rows))
