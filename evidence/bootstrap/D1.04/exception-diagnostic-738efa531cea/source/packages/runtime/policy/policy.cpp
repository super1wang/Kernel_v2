...
struct Store {
 ...
 std::shared_ptr<const PolicyConfiguration> config;std::vector<TargetIdentity> target_ids;std::vector<std::pair<PrincipalRef,std::size_t>>watch_counts;std::vector<std::shared_ptr<QueueState>>coordinators;
...
struct Action {Hold hold;ActionId id;std::shared_ptr<Verified>caller;std::shared_ptr<const GroupSnapshot>group;const ActionRequest& request;ContractDigest digest;std::vector<TargetStamp>targets;PermitBinding binding;std::uint64_t delegation;ActionStatus status=ActionStatus::Prepared;std::shared_ptr<const ActionPermit>permit;explicit Action(const ActionRequest&r):request(r),binding{{},request.envelope.operation,{},request.anchor_target,0,0,request.requested_deadline},delegation(0){};};
...
struct Watch {Hold hold;WatchKey key;std::uint64_t generation=1;std::shared_ptr<Session>session;ObservationFilter filter;TimePoint deadline;ObservationSourceIdentity source;std::vector<Entry>entries;bool closed=false;~Watch(){if(hold.counted){std::lock_guard lock(hold.store->mutex);--session->watches;for(auto&i:hold.store->watch_counts)if(i.first==session->principal){--i.second;break;}}}};
...
struct QueueState {const Session*session=nullptr;std::vector<std::shared_ptr<Frame>>queue;};
...
Result<ReadyEntry> prepare_entry(const std::shared_ptr<Session>&session,const ExecutionAccessInput&input,AccessUse use){
 ...
 Entry e{input,op->operation,{},{}};e.targets.reserve(input.actual_targets.size());e.fields.reserve(6);auto projected=v;std::uint64_t permission,delegation;
