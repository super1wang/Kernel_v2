...
struct Store {
 ...
 std::shared_ptr<const PolicyConfiguration> config;std::vector<TargetIdentity> target_ids;std::vector<std::pair<PrincipalRef,std::size_t>>watch_counts;std::vector<Coordinator*>coordinators;
...
struct Coordinator {Hold hold;std::shared_ptr<Session>session;std::shared_ptr<TransmissionStartPort>sink;std::shared_ptr<ProjectionEncoderPort>encoder;std::vector<std::shared_ptr<Frame>>queue;std::mutex pumping;~Coordinator();};
}
...
Coordinator::~Coordinator(){if(session){auto s=session->hold.store;{std::lock_guard lock(s->mutex);std::erase(s->coordinators,this);for(auto&f:queue)if(f->charged){--s->queued;s->bytes-=f->transmission->bytes().size();f->charged=false;}}}}
}
...
Result<std::unique_ptr<SendCoordinator>>SendCoordinator::create(std::shared_ptr<SessionAuthority>authority,std::shared_ptr<TransmissionStartPort>sink,std::shared_ptr<ProjectionEncoderPort>encoder){
 try{if(!owned(authority)||!owned(sink)||!owned(encoder))return failure<std::unique_ptr<SendCoordinator>>(PolicyErrc::InvalidOwner);auto s=Access::record(*authority)->session;auto c=std::make_shared<Coordinator>();c->hold.store=s->hold.store;c->hold.usage.declarations=s->hold.store->budget.queued_frames;c->session=s;c->sink=std::move(sink);c->encoder=std::move(encoder);c->queue.reserve(s->hold.store->budget.queued_frames);auto result=Access::make_unique<SendCoordinator>(c);{std::lock_guard lock(s->hold.store->mutex);auto check=current(*s);if(!check)return make_unexpected(check.error());if(s->hold.store->coordinators.size()>=s->hold.store->coordinators.capacity())return failure<std::unique_ptr<SendCoordinator>>(PolicyErrc::BudgetExceeded);check=acquire(c->hold);if(!check)return make_unexpected(check.error());s->hold.store->coordinators.push_back(c.get());}return result;
...
Result<void>SendCoordinator::enqueue_response(std::shared_ptr<const ResponseAuthorization>response){
 try{
  ...
  auto reservation=c->sink->reserve(f->transmission->bytes().size());if(!reservation)return make_unexpected(reservation.error());if(!*reservation||(*reservation)->capacity()<f->transmission->bytes().size())return deny(PolicyErrc::BudgetExceeded);f->reservation=std::move(*reservation);check=source_current(s);if(!check)return check;
