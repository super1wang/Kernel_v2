...
public:Hold hold;std::shared_ptr<Session> session;CallerAuthorityPort*issuer;std::uint64_t generation;CallerDescription description_;
 Grant(std::shared_ptr<Session>s,CallerAuthorityPort*i):session(std::move(s)),issuer(i),generation(0),description_{session->principal,session->delegated_by,std::vector<Name>(0)}{hold.store=session->hold.store;hold.usage.declarations=1;}
