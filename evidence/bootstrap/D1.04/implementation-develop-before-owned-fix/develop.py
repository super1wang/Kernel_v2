...
def run(n,args):
 r=execute(args,ROOT,out/(n+'-stdout.log'),out/(n+'-stderr.log'),600);commands.append(r);(out/'commands.json').write_text(json.dumps(commands,indent=2));print(n,r['status'],r['exit_code'],str(out),flush=True);return r['exit_code']
if '--bootstrap-red' in sys.argv:
