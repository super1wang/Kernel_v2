...
out=ROOT/'evidence/bootstrap/D1.04'/('observation-tests-'+uuid.uuid4().hex[:12]);out.mkdir()
source=out/'source';rows=[]
for name in ['packages/runtime/policy/policy.hpp','packages/runtime/policy/policy.cpp','tests/contract/authorization/fixtures.hpp','tests/contract/authorization/observation_cases.hpp','tests/compile/contracts/test_support.hpp']:
...
names=['noninvoke_permissions','page_binding_dimensions','source_store_lifetime']
main='#include "tests/contract/authorization/observation_cases.hpp"\nint main(int argc,char** argv){try {CHECK(argc==2);std::string name=argv[1];\n'
for n in names:main+='if(name=="'+n+'"){policy_test::observation_cases::'+n+'();return 0;}\n'
...
save_json(out/'result.json',dict(status='Passed' if ok else 'Failed',kind='three-observation-bootstrap-tests',source=rows))
