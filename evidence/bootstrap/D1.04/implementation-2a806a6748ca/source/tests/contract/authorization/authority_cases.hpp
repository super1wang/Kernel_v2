...
struct MovingAuthentication final : TrustedAuthenticationPort {
  ...
  explicit MovingAuthentication(TimePoint deadline)
      : payload{principal(), PrincipalKind::User, {}, {rules(), deadline, false}, deadline} {}
...
inline void returned_auth_material() {
  ...
  CHECK(observed_buffer_freed); // 先证明实际delete观察通道有效。
  observed_buffer = 0;
  Env env;
  ...
  CHECK(session);
  CHECK(observed_buffer_freed); // 不解引用已销毁的元素；检查当前实现复制后释放临时返回材料。
  observed_buffer = 0;
