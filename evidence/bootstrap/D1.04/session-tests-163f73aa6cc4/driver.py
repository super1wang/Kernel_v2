...
out=ROOT/'evidence/bootstrap/D1.04'/('session-tests-'+uuid.uuid4().hex[:12]);out.mkdir()
source=out/'source';rows=[]
for name in ['packages/runtime/policy/policy.hpp','packages/runtime/policy/policy.cpp','tests/contract/authorization/fixtures.hpp','tests/contract/authorization/session_cases.hpp','tests/compile/contracts/test_support.hpp']:
...
names=['session_isolation','service_principal','recursive_delegation_denied','delegation_shrink','operation_exact_contract','target_frozen_set']
main='#include "tests/contract/authorization/session_cases.hpp"\nint main(int argc,char** argv){try {CHECK(argc==2);std::string name=argv[1];\n'
for n in names:main+='if(name=="'+n+'"){policy_test::session_cases::'+n+'();return 0;}\n'
...
build=ROOT/'build'/('d1.04-session-'+uuid.uuid4().hex[:8]);commands=[]
def run(name,argv):
...
save_json(out/'result.json',dict(status='Passed' if ok else 'Failed',kind='six-session-bootstrap-tests',source=rows))
