...
out=ROOT/'evidence/bootstrap/D1.04'/('queue-lifetime-probe-'+uuid.uuid4().hex[:12]);out.mkdir()
source=out/'source';rows=[]
for name in ['packages/runtime/policy/policy.hpp','packages/runtime/policy/policy.cpp','tests/contract/authorization/fixtures.hpp','tests/contract/authorization/send_cases.hpp','tests/compile/contracts/test_support.hpp']:
    p=source/name;p.parent.mkdir(parents=True,exist_ok=True);p.write_bytes(((ROOT/'evidence/bootstrap/D1.04/implementation-143f7ea6042d/source'/name) if (ROOT/'evidence/bootstrap/D1.04/implementation-143f7ea6042d/source'/name).exists() else ROOT/name).read_bytes());rows.append(dict(path=name,sha256=sha_file(p),size=p.stat().st_size))
save_json(out/'source.json',rows)
(out/'driver.py').write_bytes(Path(__file__).read_bytes())
names=['close_control','unknown_cleanup']
main=r'''#include "tests/contract/authorization/send_cases.hpp"
int main(int argc,char** argv){try {using namespace policy_test;CHECK(argc==2);Env env;auto sink=std::make_shared<Sink>();auto send=send_cases::sender(env.session,sink);CHECK(send->enqueue_response(send_cases::response(env)));CHECK(send->enqueue_response(send_cases::response(env)));CHECK(sink->pending==4);if(std::string(argv[1])=="close_control"){CHECK(env.session->close());}else{sink->result=StartResult::Unknown;CHECK(send->start_next()==StartResult::Unknown);CHECK(!env.caller->view().revalidate());}std::cout<<"pending="<<sink->pending<<" size="<<sink->size<<"\n";CHECK(sink->pending==0);return 0;}catch(const std::exception&e){std::cerr<<e.what()<<"\n";return 1;}}
'''
