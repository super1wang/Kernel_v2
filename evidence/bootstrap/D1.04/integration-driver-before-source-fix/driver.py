...
out = ROOT / 'evidence/bootstrap/D1.04' / ('integration-' + uuid.uuid4().hex[:12])
out.mkdir(parents=True, exist_ok=False)
spec_path = 'tests/runs/d1.04-win-msvc-debug.json'
...
def archive_materials():
    ...
    for name in ('CMakeCache.txt', 'toolchain-Debug.json', 'ock-target-graph.json', 'CTestTestfile.cmake'):
        p = ROOT / build / name
        if p.is_file():
            files.append(p)
    materials = []
...
result = {'status': 'Failed', 'build_dir': build, 'kind': 'bootstrap-integration-only'}
try:
    ...
    binary = ROOT / build / 'tests/contract/authorization/Debug/ock_policy_tests.exe'
    actual_list = run('native-list', [str(binary), '--list']).read_text(encoding='utf-8').splitlines()
    expected = sorted(c['id'] for c in read_json(spec['expected_manifest'])['cases'] if '.policy.' in c['id'])
    assert len(expected) == 35 and sorted(actual_list) == expected
    junit = out / 'policy-junit.xml'
    run('ctest-policy', ['ctest', '--test-dir', build, '-C', 'Debug', '-R', '^T(07|19|20)[.]policy[.]', '--output-on-failure', '--no-tests=error', '--output-junit', str(junit)])
    cases = junit_cases(junit)
    assert sorted(c['name'] for c in cases) == expected and all(c['status'] == 'Passed' for c in cases)
    changed = [r['path'] for r in rows if not (ROOT/r['path']).is_file() or sha_file(ROOT/r['path']) != r['sha256']]
    result.update(status='Passed' if not changed else 'SourceChanged', cases=cases, source_changed=changed)
