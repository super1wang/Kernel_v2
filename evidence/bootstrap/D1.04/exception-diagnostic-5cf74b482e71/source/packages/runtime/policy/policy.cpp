...
struct Store {
 ...
 std::shared_ptr<const PolicyConfiguration> config;std::vector<TargetIdentity> target_ids;std::vector<std::pair<PrincipalRef,std::size_t>>watch_counts;
...
#undef POLICY_STATE
struct WatchAuthorization::State{std::shared_ptr<detail::Watch>value;};WatchAuthorization::WatchAuthorization(std::shared_ptr<State>s):state_(std::move(s)){}WatchAuthorization::~WatchAuthorization(){auto w=state_->value;std::lock_guard lock(w->hold.store->mutex);w->closed=true;}
...
Result<PolicyAssembly> PolicyStore::create(PolicyBudget b,const PolicyConfiguration&input,std::shared_ptr<TrustedAuthenticationPort>auth,std::shared_ptr<ClockPort>clock,std::shared_ptr<TrustedGroupDigestPort>digest,std::shared_ptr<ExecutionAccessSourcePort>source){
 try{
  ...
  auto s=std::make_shared<Store>();s->budget=b;s->watch_counts.reserve(b.active_watches);s->config=std::make_shared<const PolicyConfiguration>(input);s->used=usage;s->auth=std::move(auth);s->clock=std::move(clock);s->digest=std::move(digest);s->source=std::move(source);s->source_id=sid;
...
Result<bool>SendCoordinator::unsubscribe(const WatchAuthorization&authorization){auto c=state_->value;auto s=c->session->hold.store;auto w=Access::record(authorization);std::lock_guard lock(s->mutex);if(s->closed)return failure<bool>(PolicyErrc::StoreClosed);if(w->session!=c->session||w->closed||c->session->closed)return false;w->closed=true;return true;}
