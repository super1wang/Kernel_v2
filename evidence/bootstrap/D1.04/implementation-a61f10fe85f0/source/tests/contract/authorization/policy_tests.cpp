...
void budget_contract() {
  {Env e;e.auth->identity.kind=static_cast<PrincipalKind>(-1);CHECK(!e.assembly.store->open({{std::byte{7}}},{rules(),e.auth->identity.deadline,false}));}
...
void subscription_scope_atomic() {
  {Env e;ObservationFilter bad{{{id<foundation::TaskId>()}},{},{static_cast<ObservationTopic>(-1)}};CHECK(!e.session->observations()->subscribe(*e.caller,bad));}
