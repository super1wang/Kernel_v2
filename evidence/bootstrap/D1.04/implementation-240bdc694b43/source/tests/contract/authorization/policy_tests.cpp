...
void target_issued_identity(){auto cfg=configuration();cfg.targets[1].rules.clear();Env e({},cfg);auto a=e.session->targets()->resolve(e.caller->view(),target());CHECK(a);auto b=e.session->targets()->resolve(e.caller->view(),target(2));CHECK(!b);auto c=e.session->targets()->resolve(e.caller->view(),target(3));CHECK(!c);CHECK(b.error().code()==c.error().code());}
...
int main(int argc,char** argv){
  try{
    CHECK(argc==2);
    const std::map<std::string,void(*)()> cases{{"T07.policy.owned_inputs_budget",budget_contract},{"T07.policy.authentication_source",authentication_source},{"T07.policy.expiry_boundary",clock_boundaries},{"T07.policy.scope_four_way",scope_four_way},{"T07.policy.target_issued_identity",target_issued_identity}
#ifndef OCK_POLICY_STAGE
