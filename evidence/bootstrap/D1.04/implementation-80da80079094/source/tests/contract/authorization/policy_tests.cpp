...
void budget_contract() {
  for (std::size_t keys : {1u, 2u, 3u}) {
    PolicyBudget b; b.text_bytes = configured_text() + keys * selector_text(operation());
    Env e(b);
    auto response = e.session->observations()->get(*e.caller, {id<foundation::TaskId>()}, AccessUse::GetSummary);
    CHECK(bool(response) == (keys >= 2));
    if (response) {
      auto send = SendCoordinator::create(e.session, std::make_shared<Sink>(), std::make_shared<Encoder>()); CHECK(send);
  ...
  for (std::size_t keys : {1u, 2u, 3u}) {
    ...
    auto watch = e.session->observations()->subscribe(*e.caller, filter); CHECK(watch);
    auto send = SendCoordinator::create(e.session, std::make_shared<Sink>(), std::make_shared<Encoder>()); CHECK(send);
    CHECK(bool((*send)->enqueue(**watch, {e.source->rows[0].second.summary, ObservationTopic::Progress, false})) == (keys >= 3));
    CHECK((*send)->unsubscribe(**watch)); watch->reset();
...
void exception_atomicity() {
  {
    PolicyBudget b; b.sessions = 2; b.active_watches = 1; b.watches_per_session = 1; b.watches_per_principal = 1;
    Env e(b); e.auth->throw_allocation = true;
    CHECK(!e.assembly.store->open({{std::byte{7}}}, {rules(), e.auth->identity.deadline, false}));
    e.auth->throw_allocation = false;
    auto second = e.assembly.store->open({{std::byte{7}}}, {rules(), e.auth->identity.deadline, false}); CHECK(second);
    ObservationFilter filter{{{id<foundation::TaskId>()}}, {}, {ObservationTopic::Progress}};
    ...
    auto watch = e.session->observations()->subscribe(*e.caller, filter); CHECK(watch);
    CHECK(!e.session->observations()->subscribe(*e.caller, filter));
    watch->reset(); CHECK(e.session->observations()->subscribe(*e.caller, filter));
  }
