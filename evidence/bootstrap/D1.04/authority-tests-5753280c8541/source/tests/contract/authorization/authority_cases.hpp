...
inline thread_local std::uintptr_t observed_buffer = 0;
inline thread_local bool observed_buffer_freed = false;
inline void observe_returned_buffer_free(void* p) noexcept {
  if (observed_buffer && reinterpret_cast<std::uintptr_t>(p) == observed_buffer)
    observed_buffer_freed = true;
}
struct StopObserving {
  ~StopObserving() { observed_buffer = 0; }
};
struct MovingAuthentication final : TrustedAuthenticationPort {
  AuthenticatedIdentity payload;
  explicit MovingAuthentication(TimePoint deadline)
      : payload{principal(), PrincipalKind::User, {}, {{rules().front()}, deadline, false}, deadline} {}
  Result<AuthenticatedIdentity> authenticate(const AuthenticationAttempt&) override {
    observed_buffer = reinterpret_cast<std::uintptr_t>(payload.ceiling.rules.data());
    observed_buffer_freed = false;
    return std::move(payload);
  }
};
inline void returned_auth_material() {
  StopObserving guard;
  {
    // 锁定MSVC的大块vector会调整data地址；使用小块公开分配避免私有对齐布局。
    static_assert(sizeof(ScopeRule) < 4096);
    std::vector<ScopeRule> positive{rules().front()};
    observed_buffer = reinterpret_cast<std::uintptr_t>(positive.data());
    observed_buffer_freed = false;
  }
  require(observed_buffer_freed, "small-buffer delete observer positive control");
  observed_buffer = 0;
  Env env;
  auto auth = std::make_shared<MovingAuthentication>(env.auth->identity.deadline);
  auto assembly = PolicyStore::create({}, configuration(), auth, env.clock, env.digest, env.source);
  CHECK(assembly);
  auto session = assembly->store->open({{std::byte{7}}}, {rules(), env.auth->identity.deadline, false});
  CHECK(session);
  require(observed_buffer_freed, "returned auth buffer released after independent snapshot");
  observed_buffer = 0;
  auto caller = (*session)->verify({principal(), {}, {}}); CHECK(caller);
  CHECK((*session)->prepare(**caller, env.request())); // 冻结授权仍实际存在。
}
...
inline void inputs_ownership() {
  ...
  auto& aliased_scope = scope.rules[0].permissions;
  ...
  CHECK(PolicyStore::create({}, configuration(), helpers.auth, helpers.clock, helpers.digest, helpers.source));
  returned_auth_material();
}
} // namespace policy_test::authority_cases
