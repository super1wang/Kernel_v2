...
struct Sink final:TransmissionStartPort {
 ...
 StoreId last_store{};std::array<std::byte,32768> data{};std::size_t size=0;unsigned starts=0;
 ...
 StartResult start_now(const PreparedTransmission&frame,TransmissionReservation&r)noexcept override{
  ...
  own->used=true;last_store=frame.binding().store;for(auto b:frame.bytes())data[size++]=b;return result;
 }
};
struct Env {
