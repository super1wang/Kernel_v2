...
def run(n,args):
 r=execute(args,ROOT,out/(n+'-stdout.log'),out/(n+'-stderr.log'),30 if '--quick' in sys.argv else 600);commands.append(r);(out/'commands.json').write_text(json.dumps(commands,indent=2));print(n,r['status'],r['exit_code'],str(out),flush=True)
