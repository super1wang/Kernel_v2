...
|---|---|
| PolicyStore | StoreId、全部预算、私有饱和发行器、当前权限generation、规范化配置、固定auth/clock/digest/source owner、仲裁及有界活动索引；create创建 |
...
| WatchAuthorization | WatchKey/generation、固定Session owner、规范化ObservationFilter、deadline、Active/Closed状态及有界配额归属；subscribe创建 |
| ProjectionSnapshot | 自有kind及与之匹配的variant值；ObservationAuthorization创建get/list投影，SendCoordinator创建已重验hint投影 |
| ResponseAuthorization | ResponseId、固定Session owner、use、deadline、原始ProjectionSnapshot、逐投影项目的ExecutionRef/精确OperationSelector/真实owner/所有目标实例与生命周期/实际输出字段、权限/委托generation、Fresh/Queued/Started/Dropped/Unknown终态；get/list创建 |
