...
void *operator new(std::size_t size) {
  if(measure_allocations)largest_allocation=(std::max)(largest_allocation,size);
...
void budget_contract() {
  {Env e;auto oversized=configuration().principals[0];auto sample=oversized.rules[0];oversized.rules.assign(4097,sample);largest_allocation=0;measure_allocations=true;auto result=e.assembly.administration->replace_principal_policy(oversized);measure_allocations=false;CHECK(!result);CHECK(largest_allocation<oversized.rules.size()*sizeof(ScopeRule));CHECK(e.prepare());}
...
void authentication_source() {
  Env e;
  CHECK(e.caller->view().revalidate());
  CHECK(!e.assembly.store->open({{std::byte{8}}},
                                {rules(), e.auth->identity.deadline, false}));
  CHECK(!e.session->verify({principal(2), {}, {}}));
}
...
void scope_four_way() {
  ...
  for (int side = 0; side < 4; ++side) {
    auto cfg = configuration();
    if (side == 0)
      cfg.principals[0].rules.clear();
    if (side == 2)
      cfg.operations[0].module_rules.clear();
    if (side == 3)
      cfg.targets[0].rules.clear();
    Env e({}, cfg);
    if (side == 1)
      CHECK(e.session->restrict_delegation(
          {{}, e.auth->identity.deadline, false}));
    CHECK(!e.prepare());
  }
}
