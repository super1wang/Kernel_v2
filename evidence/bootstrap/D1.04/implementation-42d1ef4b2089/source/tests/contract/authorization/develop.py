...
def run(n,args):
 r=execute(args,ROOT,out/(n+'-stdout.log'),out/(n+'-stderr.log'),600);commands.append(r);(out/'commands.json').write_text(json.dumps(commands,indent=2));print(n,r['status'],r['exit_code'],str(out),flush=True)
 if r['status']!='Exited' or r['process_tree']['active_after']!=0:raise RuntimeError('Owned process did not exit cleanly: '+n)
 return r['exit_code']
if '--stage' in sys.argv:
 includes=[ROOT,ROOT/'tests/compile/contracts',ROOT/'packages/contracts/include',ROOT/'packages/foundation/include',ROOT/'build/d0.06-a/cache/sources/expected-fe3b18aecb84/include']
 lines=['cmake_minimum_required(VERSION 3.25)','project(PolicyStage LANGUAGES CXX)',f'add_executable(policy_stage "{ROOT.as_posix()}/packages/runtime/policy/policy.cpp" "{ROOT.as_posix()}/tests/contract/authorization/policy_tests.cpp")','target_compile_features(policy_stage PRIVATE cxx_std_20)','target_compile_definitions(policy_stage PRIVATE OCK_POLICY_STAGE)','target_compile_options(policy_stage PRIVATE /utf-8 /EHsc /Zc:__cplusplus /permissive-)', 'target_include_directories(policy_stage PRIVATE '+' '.join('"'+p.as_posix()+'"' for p in includes)+')']
 ...
 failed=False
 for name in (out/'list-stdout.log').read_text().splitlines():failed=bool(run(name,[str(binary),name])) or failed
 sys.exit(int(failed))
