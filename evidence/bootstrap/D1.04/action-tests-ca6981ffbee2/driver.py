...
out=ROOT/'evidence/bootstrap/D1.04'/('action-tests-'+uuid.uuid4().hex[:12]);out.mkdir()
source=out/'source';rows=[]
for name in ['packages/runtime/policy/policy.hpp','packages/runtime/policy/policy.cpp','tests/contract/authorization/fixtures.hpp','tests/contract/authorization/action_cases.hpp','tests/compile/contracts/test_support.hpp']:
    p=source/name;p.parent.mkdir(parents=True,exist_ok=True);p.write_bytes((ROOT/name).read_bytes());rows.append(dict(path=name,sha256=sha_file(p),size=p.stat().st_size))
save_json(out/'source.json',rows)
(out/'driver.py').write_bytes(Path(__file__).read_bytes())
names=['permit_origin_binding','permit_once','permit_concurrent','revoke_before_consume','consume_before_revoke','cancel_arbitration','group_members_complete','group_substitution']
main='#include "tests/contract/authorization/action_cases.hpp"\nint main(int argc,char** argv){try {CHECK(argc==2);std::string name=argv[1];\n'
for n in names:main+='if(name=="'+n+'"){policy_test::action_cases::'+n+'();return 0;}\n'
...
build=ROOT/'build'/('d1.04-action-'+uuid.uuid4().hex[:8]);commands=[]
def run(name,argv):
...
save_json(out/'result.json',dict(status='Passed' if ok else 'Failed',kind='eight-action-bootstrap-tests',source=rows))
