...
struct Digest final:TrustedGroupDigestPort {
 ...
 Result<ContractDigest> fingerprint(const GroupSnapshot&g)override {if(throw_allocation)throw std::bad_alloc();
  ContractDigest d{};std::uint64_t value=1469598103934665603ULL;
  auto byte=[&](unsigned char b){value=(value^b)*1099511628211ULL;};
  auto count=[&](std::uint64_t n){for(unsigned i=0;i<8;++i)byte(static_cast<unsigned char>(n>>(8*i)));};
  auto chars=[&](std::string_view text){count(text.size());for(unsigned char c:text)byte(c);};
  auto selector=[&](const OperationSelector&o){chars(o.operation.name.view());chars(o.operation.version.text());for(auto b:o.contract.bytes)byte(std::to_integer<unsigned char>(b));};
  byte(1);selector(g.envelope());for(auto b:g.anchor_target().bytes)byte(b);count(g.members().size());
  for(auto&m:g.members()){byte(2);selector(m.operation);count(m.targets.size());for(auto t:m.targets)for(auto b:t.bytes)byte(b);}
  if(!collision)for(unsigned i=0;i<8;++i)d.bytes[i]=std::byte(value>>(8*i));return d;
 }
};
struct Source final:ExecutionAccessSourcePort {
 ...
 unsigned scans=0;bool find_failure=false,scan_failure=false;
 ...
 Result<ExecutionAccessInput> find(ExecutionRef e)override{if(find_failure)throw std::bad_alloc();for(auto&r:rows)if(r.second.summary->value().execution==e)return r.second;return make_unexpected(policy_error(PolicyErrc::TargetUnavailable));}
 Result<AccessScanPage> scan(const AccessScanRequest&r)override {
  ++scans;if(scan_failure)throw std::bad_alloc();AccessScanPage p{{},{},source_id.host,name("retained")};std::size_t seen=0;
...
struct Encoder final:ProjectionEncoderPort {bool throw_allocation=false;
 Result<std::vector<std::byte>> encode(const ProjectionSnapshot&p,std::size_t cap)override {
  if(throw_allocation)throw std::bad_alloc();if(cap<2)return make_unexpected(policy_error(PolicyErrc::BudgetExceeded));return std::vector<std::byte>{std::byte{0x51},std::byte(static_cast<unsigned>(p.kind()))};
 }
};
struct Sink final:TransmissionStartPort {
 struct Reservation final:TransmissionReservation{Sink* owner;std::size_t count;bool used=false;~Reservation(){if(!used)owner->pending-=count;}Reservation(Sink*p,std::size_t n):owner(p),count(n){}std::size_t capacity()const noexcept override{return count;}};
 StoreId last_store{};std::array<std::byte,32768> data{};std::size_t size=0,pending=0;unsigned starts=0;
 StartResult result=StartResult::Started;bool reject_reserve=false,throw_allocation=false;
 std::function<void()> before_reserve_return;
 Result<std::unique_ptr<TransmissionReservation>> reserve(std::size_t n)override{if(throw_allocation)throw std::bad_alloc();
  if(reject_reserve||size>data.size()||pending>data.size()-size||n>data.size()-size-pending)return make_unexpected(policy_error(PolicyErrc::Busy));
  auto reservation=std::make_unique<Reservation>(this,n);pending+=n;if(before_reserve_return)before_reserve_return();return std::unique_ptr<TransmissionReservation>(std::move(reservation));
 }
 StartResult start_now(const PreparedTransmission&frame,TransmissionReservation&r)noexcept override{
  ...
  if(frame.bytes().size()>data.size()-size)return StartResult::NotStarted;own->used=true;pending-=own->count;last_store=frame.binding().store;for(auto b:frame.bytes())data[size++]=b;return result;
 }
};
struct Env {
