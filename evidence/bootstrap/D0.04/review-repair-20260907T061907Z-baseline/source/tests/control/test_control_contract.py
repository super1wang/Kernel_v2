...
class ControlContracts(unittest.TestCase):
 ...
 def test_T19_control_client_stale_snapshot_and_event(self):
  c=setup();s=sub(c);c.ack('c1',s);client=Client();client.accept_snapshot(c.get('c1',eid('e1')));c.update(eid('e1'),phase='Finalizing')
 ...
 def test_T19_control_disconnect_and_reconnect(self):
  ...
  client=Client();client.accept_snapshot(c.get('c1',eid('e1')));self.assertFalse(client.accept_event({'params':{'host_incarnation':'new'}}));self.assertTrue(client.needs_snapshot)
