...
O=R/'evidence/B6/semantic-precision-912cb75';D=O/'matrix-02';D.mkdir(exist_ok=True);commands=[];reports=[]
for p in ('debug','release','asan'):
 print('START FORMAL '+p,flush=True)
 r=execute([sys.executable,'-X','utf8','tools/evidence/run.py','tests/runs/b6-semantic-v2-win-msvc-'+p+'.json'],R,D/(p+'-stdout.log'),D/(p+'-stderr.log'),2400)
 ...
 reports.append(str(paths[0]));(O/'matrix-reports-02.json').write_text(json.dumps(reports,indent=2)+'\n')
