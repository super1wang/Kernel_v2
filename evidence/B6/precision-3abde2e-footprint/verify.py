...
save(O/'acceptance.json',{'format':'ock.b6-final-precision-footprint-acceptance/1','status':'Passed','created_at':datetime.now(timezone.utc).isoformat(),'production_commit':source['production_commit'],'validation_checkout':subprocess.check_output(['git','rev-parse','HEAD'],cwd=R,text=True).strip(),'source_binding':item(O/'source-binding.json'),'expected':item(O/'expected.json'),'commands':item(O/'commands.json'),'budgets':item(R/'footprint-budgets.json'),'previous_budgets':item(O/'previous-budgets.json'),'budget_limits_and_pilots_unchanged':True,'verified_source_inputs':len(source['inputs']),'reports':checks,'pruning':pruning,'B7_entry':'GO','G4':'NotStarted','scope':'仅补 CoreContracts 布局变化触发的 footprint；保留既有代码与功能验收，未重跑历史全矩阵；不是 GUI、设备或物理机放行。'})
