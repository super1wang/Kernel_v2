"""B6 最终线性化收口 footprint 来源、预算与原始证据复核。"""
...
save(O/'footprint-integrity.json',{'status':'Passed','checked_at':datetime.now(timezone.utc).isoformat(),'production_commit':source['production_commit'],'source_binding':item(O/'source-binding.json'),'expected':item(O/'expected.json'),'commands':item(O/'commands.json'),'budgets':item(R/'footprint-budgets.json'),'previous_budgets':item(O/'previous-budgets.json'),'budget_limits_and_pilots_unchanged':True,'verified_source_inputs':len(source['inputs']),'reports':checks,'pruning':pruning,'scope':'B6 最终线性化收口同一来源的 Native/Embedded 原预算 footprint 完整性；不单独授予最终放行。'})
