...
消费者对同一个注册的 typed Compute 连续执行 40 次 Invoke/Submit 对比，提交后修改调用者输入，确认异步结果仍对应原始值；终态上限 16、记录上限 32，运行跨过缓存保留窗口，最后验证 Host quiescent 与模块停止。它证明公开 SDK 接线与基本回收，不代替完整父子/取消/资源组合、线程峰值或 G3 footprint。
