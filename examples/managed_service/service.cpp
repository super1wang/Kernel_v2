...
Result<std::unique_ptr<Service>> Service::create(std::string sid) {
  ...
  module.register_operations=[](registry::Registrar& registrar) {
    ...
    contracts::DefinitionInput definition{operation(),{},{false,false,false,name("cpu"),name("managed")},contracts::AtomicMode::PureCompute,{name("sample.use")},
      R"({"format":"ock.command-docs/1","purpose":"Bounded managed computation","counterexamples":["Negative amount is invalid"],"coordinates":"not applicable","position_mode":"not applicable","impact_scope":"returned value only","id_sources":"current host catalog","cancellation":"cooperative stop returns observed intent","retry":{"natural_idempotence":"yes","framework_deduplication":"not provided","device_deduplication":"not applicable"},"durability":"Volatile","result_phases":"ReadCompleted, FailedBeforeApply or CancelledBeforeApply after acceptance","error_repair":"correct input and retry"})"};
