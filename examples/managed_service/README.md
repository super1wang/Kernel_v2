...
开发 SDK `0.1.0-dev.8 / B6Subset` 支持将本目录复制到仓库之外，使用迁移后的安装目录独立构建。`find_package(OCK)` 取得 Control、AdapterLocalIPC、CpuPool 以及 `OCK_SCHEMA_DIR` 指向的已安装 RPC Schema；消费者不读取生产源码目录。
