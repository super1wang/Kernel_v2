...
public:
  static Result<std::shared_ptr<ProtocolObservationTransport>> create(std::shared_ptr<FrameSink>,
      FramePriority = FramePriority::Notification);
...
private:
  explicit ProtocolObservationTransport(std::shared_ptr<FrameSink> sink,FramePriority priority)
      : sink_(std::move(sink)),priority_(priority) {}
  std::shared_ptr<FrameSink> sink_;
  FramePriority priority_;
};
}
