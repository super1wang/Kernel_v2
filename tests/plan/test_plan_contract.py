...
class PlanContracts(unittest.TestCase):
    ...
    def test_T04_plan_bound_domain_and_target_validate_full_schema(self):
        atomic=dict(kind='atomic',id='a',preconditions={'revision':'7'},bindings=[{'to':'/domain','value':ref('$input','/domain')}],steps=[call()],exports={},out='group')
        valid={'provider':'settings','id':'settings.machine-a'}
        PlanChecker().check(dict(base([atomic]),inputs={'domain':valid}))
        invalid=[{'provider':1,'id':[]},{'provider':'settings','id':''},{'provider':'settings','id':'x'*97},{'provider':'设定','id':'valid'},{'provider':'settings','id':'ok','extra':1},None,[]]
        for domain in invalid:
            with self.subTest(domain=domain):
                with self.assertRaises(ContractError):PlanChecker().check(dict(base([atomic]),inputs={'domain':domain}))
                edit=call(name='settings.set_limit');edit['args']={'axis':'x','limit':100};edit['bindings']=[{'to':'/target','value':ref('$input','/domain')}]
                with self.assertRaises(ContractError):PlanChecker().check(dict(base([edit]),inputs={'domain':domain}))
        edit=call(name='settings.set_limit');edit.update(args={'axis':'x','limit':100},target=valid);PlanChecker().check(base([edit]))
