...
# Wire允许绑定前省略叶；完成绑定后恢复同一Schema的全部必需字段。
BOUND_DOMAIN_SCHEMA={**SCHEMA['$defs']['atomic']['properties']['domain'],'required':['provider','id']}
BOUND_DOMAIN_VALIDATOR=Draft202012Validator(BOUND_DOMAIN_SCHEMA)
MAX_U64=2**64-1
        ...
        for raw in block['steps']:
            ...
            if kind=='call':
                ...
                if shape=='state_edit':
                    ...
                    require(next(BOUND_DOMAIN_VALIDATOR.iter_errors(target),None) is None,'BoundTargetSchema')
                ...
                require('domain' in bound,'AtomicDomainRequired')
                require(next(BOUND_DOMAIN_VALIDATOR.iter_errors(bound['domain']),None) is None,'BoundAtomicDomainSchema')
