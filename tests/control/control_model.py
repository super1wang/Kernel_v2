...
class Client:
 ...
 def __init__(self):self.views={};self.host=None;self.needs_snapshot=True;self.sequences={};self.streams={};self.ack_host=None;self.snapshot_versions={}
 def establish(self,ack):
  host=ack['host_incarnation']
  if self.host is not None and self.host!=host:self.views.clear();self.sequences.clear();self.streams.clear();self.snapshot_versions.clear()
  self.ack_host=host;self.host=host
  self.streams[ack['subscription_id']]=ack['stream_generation'];self.needs_snapshot=True
 def retire(self,subscription_id,stream_generation):
  if self.streams.get(subscription_id)!=stream_generation:return False
  del self.streams[subscription_id];self.sequences.pop((subscription_id,stream_generation),None);return True
 def disconnect(self):
  self.streams.clear();self.sequences.clear();self.snapshot_versions.clear();self.views.clear()
  self.host=None;self.ack_host=None;self.needs_snapshot=True
 ...
 def accept_snapshot(self,snapshot):
  ...
  if self.host is not None and host!=self.host:self.views.clear();self.sequences.clear();self.snapshot_versions.clear()
  ...
  version=int(snapshot['observation_version'])
  if old and int(old['observation_version'])>version:return False
  # 部分通知的版本不能代表整个执行投影已完整；同版get可以补齐缺失字段。
  if old and int(old['observation_version'])==version and self.snapshot_versions.get(execution)==version and not self.needs_snapshot:return False
  self.views[execution]=deepcopy(snapshot);self.snapshot_versions[execution]=version;self.needs_snapshot=False;return True
 def accept_event(self,event):
  p=event['params']
  if p['host_incarnation']!=self.host:self.needs_snapshot=True;return False
  if p['subscription_id'] not in self.streams or self.streams[p['subscription_id']]!=p['stream_generation']:return False
