...
inline void resource_lifetime() {
  ...
  auto ephemeral=std::make_shared<ResourceLease>();std::weak_ptr<ResourceLease> non_owner=ephemeral;
  const ResourceLease* borrowed_view=ephemeral.get();
  { WorkContext borrowed({},policy::TimePoint{},budget,name("nonowner"),BorrowedResourceViews{std::span(&borrowed_view,1)});
    CHECK(borrowed.granted_resources()[0]==ephemeral.get());ephemeral.reset();CHECK(non_owner.expired());
    // 借用不延长租约寿命；此后不读取或解引用失效视图，只销毁上下文。
  }
...
inline void outcome_consistency() {
  ...
  for(unsigned n:{1,2,3})uncertain.push_back(UnknownFact{id<FactId>(n),UnknownBoundary::ExternalEffect,id<EffectId>(n),"reconcile"});
  auto original=facts(uncertain);
  uncertain.push_back(ResolutionRecord{id<FactId>(4),id<FactId>(1),Determination::NotApplied,"receipt"});
  auto resolved=facts(uncertain);
  OutcomeValidation evidence{publication,{},fact_budget()};
  const auto accepts=[&](const std::shared_ptr<const KnownFacts>& known,std::vector<FactId> ids){
    return bool(Outcome<int>::validate(Indeterminate{std::move(ids)},known,EvidenceState::PersistenceUncertain,conditions(),evidence));
  };
  CHECK(accepts(original,{id<FactId>(1),id<FactId>(2),id<FactId>(3)}));
  CHECK(accepts(original,{id<FactId>(3),id<FactId>(1),id<FactId>(2)}));
  CHECK(!accepts(original,{id<FactId>(1),id<FactId>(1),id<FactId>(3)}));
  CHECK(!accepts(original,{id<FactId>(1),id<FactId>(2)}));
  CHECK(!accepts(original,{id<FactId>(1),id<FactId>(2),id<FactId>(9)}));
  CHECK(accepts(resolved,{id<FactId>(2),id<FactId>(3)}));
  CHECK(!accepts(resolved,{id<FactId>(1),id<FactId>(3)}));
  CHECK(!accepts(resolved,{id<FactId>(2),id<FactId>(4)}));
  for(const auto& known:{facts(),original,resolved}){
    const auto old=ock::contracts::detail::unresolved(*known);
    CHECK(ock::contracts::detail::count_unresolved(*known)==old.size());
    for(unsigned n:{1,2,3,4,9})CHECK(ock::contracts::detail::is_unresolved(*known,id<FactId>(n))==
        (std::find(old.begin(),old.end(),id<FactId>(n))!=old.end()));
  }
}
