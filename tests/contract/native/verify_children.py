...
CONTROLS = {
    ...
    "no_self_wait": {
        ...
        "force_inline_call": ('void rejected(NativeBound<Value,Value>& b) { b.force_inline(); }', 'C2039'),
    },
}
...
def allocation(raw, case):
    ...
    assert isinstance(report['coverage'], dict) and report['coverage']['cpp_new'] is True
    assert type(report['coverage']['debug_crt']) is bool
    assert type(report['coverage']['asan_allocator_hooks']) is bool
    effective = ('unavailable' if not report['coverage']['debug_crt'] else
                 'unobserved_asan_intercepted' if report['coverage']['asan_allocator_hooks'] else 'full')
    assert report['coverage']['debug_crt_effective'] == effective
    assert report['coverage']['modules'] and report['coverage']['blind_spots']
    assert report['coverage']['window'] and report['coverage']['scope']
    assert isinstance(report['samples'], list) and report['samples']
    for sample in report['samples']:
        assert isinstance(sample['name'], str) and sample['name']
        assert type(sample['cpp']) is int and sample['cpp'] >= 0
        assert type(sample['success']) is bool
        if report['coverage']['debug_crt']:
            assert type(sample['crt']) is int and sample['crt'] >= 0
        else:
            assert sample['crt'] is None
        for key in ('asan_allocations', 'asan_frees'):
            if report['coverage']['asan_allocator_hooks']:
                assert type(sample[key]) is int and sample[key] >= 0
            else:
...
def main():
    ...
    if short == 'private_dispatch':
        ...
        assert exited(terminated, 86)
        assert b'locked_result_noexcept_transport_terminated' in stderr
        structure['throwing_transport_exit'] = terminated['exit_code']
    if short in ALLOCATIONS:
        ...
        if short == 'allocation_probe':
            entries = ('new', 'new_array', 'aligned_new', 'aligned_array', 'nothrow_new',
                       'nothrow_array', 'nothrow_aligned', 'nothrow_aligned_array',
                       'malloc', 'calloc', 'realloc', 'aligned_malloc')
            assert tuple(s['name'] for s in report['samples'][:12]) == entries
            missing = report['coverage']['crt_missing_positive_entries']
            assert missing == ([s['name'] for s in report['samples'][:12] if s['crt'] == 0]
                               if report['coverage']['debug_crt'] else None)
            if report['coverage']['asan_allocator_hooks']:
                assert all(s['asan_allocations'] == 1 for s in report['samples'][:12])
                hook_failed, _, hook_stderr = command('hook-registration-failure', [args.binary, '--asan-hook-registration-failure'])
                save_json(out / 'hook-registration-failure.json', hook_failed)
                assert exited(hook_failed, 87)
                assert b'native_asan_hook_registration_failed' in hook_stderr
            red, raw, _ = command('probe-rejection', [args.binary, '--allocation-injected-red'])
            save_json(out / 'probe-rejection.json', red)
            assert exited(red, 1)
            injected = allocation(raw, '--allocation-injected-red')
            ...
            if report['coverage']['asan_allocator_hooks']:
                assert any(s['name'] == 'intentional_single_allocation' and s['asan_allocations'] == 1
                           for s in injected['samples'])
        ...
        for unit in source.parent.iterdir():
            if unit.suffix in ('.cpp', '.hpp'):
                assert not re.search(r'#\s*include\s*[<"][^">]*tests[/\\]', unit.read_text(encoding='utf-8'))
