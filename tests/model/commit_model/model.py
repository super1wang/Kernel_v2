...
class Effect:
    ...
    def send(self, application, status):
        self._check_claim_material()
        require(self.permitted and not self.recovered and not self._send_attempted, '不得无许可或在已尝试/未知恢复后盲重发')
        # 先消费权威一次状态；公开 report 的变化或外部调用异常均不能重新打开发送。
        ...
        try:
            ...
            # 调用方无法据异常证明未生效；保持稳定身份，之后只能以外部证据对账。
            self.restart()
            raise
