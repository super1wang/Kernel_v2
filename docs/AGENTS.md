...
- D1–D7 默认 Code-First：规范明确后立即实现生产代码。简单任务直接实现并配最小直接测试，不先建独立 plan、SPEC/CODE、acceptance 或 Evidence 文档。
...
- **当前动作：D1.06 Production Batch。** Host/Logging/NativeSubset 已实现，继续 footprint 缺项及集中验收；工具优化结束，D1.05 已 Passed，不重验。
