...
日期：2026-09-13。状态：实施中；本决定不代替机器验收。
...
公开 Host 接线决定（实施前补充）：HostOptions 显式 enable_state，默认关闭；开启后允许已注册的 StateEdit provider，仍经同一 Registry、Policy、HostBound 和执行后端，不引入 Runtime 对 State 的依赖。RevisionPolicy 由可信 OperationOptions 声明 ServerCapture 或 RequireExplicitRevision；客户端 InvokeOptions 只携带 expected_state（revision 与 lifecycle_generation），不能改变策略。匹配在业务进入前完成，managed 使用相同快照及 commit 路径。
