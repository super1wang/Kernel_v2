...
修订5固定精确契约映射：同一Store已安装的`OperationKey → ContractDigest`在其整个寿命中不可重绑定。replace_operation_policy对已有Key提供不同Digest时原子返回ContractMismatch，不替换配置、不消耗权限generation，也不得使旧授权或响应失效；相同Digest下的所需权限/module_rules更新仍按既定世代仲裁执行。新契约使用新的版本Key，不能同版本原地改义。

ExecutionSummary只含OperationKey，因此本包以可信初始装配保证源中每项执行的真实契约与已安装Key→Digest映射一致，并用上述稳定性保证新get/list不会在Store内部重新解释已有执行。源提供未知Key必须拒绝；不能从调用者补入Digest或把当前目录升级当成旧执行真实契约的证明。关闭并创建新Store也不能把既存执行重新标成另一契约；可信装配仍须保持真实含义，版本升级由新Key表达。此约束不新增source字段、执行索引或生产注册服务，沿用修订3同ExecutionRef授权事实不可重绑定的边界。
