...
工作包：D0.02 基线、D1.01 Foundation 实施；依据 A02、A19、A20。开发 SDK 版本为 `0.1.0-dev.1`，独立于架构文档 v3.3。

## 已存在与尚未实现

当前实际创建 21 个 `INTERFACE_LIBRARY` 合同 target，并由 CMake 的 install(EXPORT) 导出依赖。Foundation已落地版本元数据头 `ock/foundation/sdk_version.hpp` 与原语入口 `ock/foundation/foundation.hpp`；原语合同见 [Foundation](contracts/foundation.md)。其余组件保持合同阶段。

`OCK::Foundation` 的 `OCK_IMPLEMENTATION_STAGE=Implemented` 随安装导出，其余20个目标保持 `ContractBaseline`；`OCK_RUNTIME_AVAILABLE=FALSE`。`find_package(OCK CONFIG REQUIRED)` 可读取版本与候选目标元数据，`COMPONENTS Foundation` 可用于真实原语消费；`COMPONENTS Runtime` 及尚未实现的组件请求均明确拒绝。不能把合同 target 的存在当成模块实现通过。
...
版本头提供 SDK 数字/文本版本和 `runtime_available=false` 元数据。Foundation公开编译要求为 C++20，并对MSVC导出 `/utf-8 /EHsc /Zc:__cplusplus /permissive-`；其他目标自身保留原有 `/utf-8` 并经依赖链继承Foundation要求，不导出本机绝对路径。MSVC Debug 使用动态 Debug CRT，Release 使用动态 Release CRT；实际工具链和依赖锁由 D0.06 定案，本包的测量不能代替完整支持矩阵。
...
Foundation公开链接锁定的 `OCKThirdParty::expected`，安装tl::expected 1.1.0头和CC0-1.0许可证；第三方闭包与上表产品DAG分别登记。jsoncons、immer、SQLite、Asio等不泄漏到核心公开类型。依赖取得和完整性验证复用D0.06的锁定工具，关闭BUILD_TESTING后仍可安装消费。
